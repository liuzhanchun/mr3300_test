...
CDevGDI::CDevGDI(void)
{
	...
	memset(m_czMkrVal, 0, sizeof(TCHAR)*I_MKR_NUMBER*N_TEXT_LEN*I_WAVE_NUM);
	m_iActiveMkr = 0;
	m_nActiveLimit = 0;
	...
	for (i=0; i<N_MAX_MANUAL_TEXT_NUMBER; i++)
	{
		...
		m_ManualText[i].nSize = 12;//m_nTextFontSize;
		m_ManualText[i].cr = RGB(255,255,0);
	}
	...
	for (int i=0; i<I_WAVE_NUM; i++) {
		for (int j=0; j<I_MKR_NUMBER; j++) {
			_tcscpy(m_czMkrVal[i][j], _T(" "));
		}
	}
	...
	m_IsSpectrogramMkrLine = false;

	ZeroMemory(m_Rectangles, N_RECT_NUM*sizeof(tagRectangle));
	m_nRectangles = 0;

	ZeroMemory(m_ManualTable, N_TABLE_NUM*sizeof(tagManualTable));
}
...
void CDevGDI::GDI_SetGridColor(const COLORREF &GridColor)
{
	m_GridColor = GridColor;
}
...
void CDevGDI::GDI_SetLineColor(int nColor, COLORREF cr)
{
	m_LineColor[nColor] = cr;
}

COLORREF CDevGDI::GDI_GetLineColor(int nColor)
{
	return m_LineColor[nColor];
}
...
void CDevGDI::GDI_SetMkrColor(const COLORREF &mkrColor, const int &nMkrID )
{
	int i = 0;
	assert((nMkrID>=0)&&(nMkrID<I_MKR_NUMBER));
	for (; i<I_WAVE_NUM; i++)
	...
	const int &nDataLen20 /*= 0*/)
{
	...
	DrawRectangles(pDC->m_hDC, nDataLen);
	...
	for (i=0; i<I_MKR_NUMBER; i++)
	{
		if (m_bMkr[i])
		{
			if (nDataLen>0)
			{
				...
				DrawReportMkr(pDC->m_hDC, pdData, nDataLen, i, 0);
			}
			if (nDataLen2>0)
			{
				if (m_bMkrKeepMaxVal)	// 频标跟踪峰值
				...
				DrawReportMkr(pDC->m_hDC, pdData2, nDataLen2, i, 1);
			}
		}
	}
	...
	ShowManualTextForPrint(pDC->m_hDC);
	...
	const double *pdData19 /*= NULL*/, const int &nDataLen19 /*= 0*/, const double *pdData20 /*= NULL*/, const int &nDataLen20 /*= 0*/)
{
	...
	ShowManualText(memDC);
	DrawMultiText(memDC);   // 自定义文字
	DrawManualTable(memDC);	// 自定义表格
...
int CDevGDI::GetWaveLineY(const double &dData)
{
	...
	if (nYVal<m_rect.top+m_iBorderGridTop-2)
	{
		nYVal = m_rect.top+m_iBorderGridTop-2;
	}
	else if (nYVal>m_rect.bottom-m_iBorderGridBottom)
	{
		nYVal = m_rect.bottom-m_iBorderGridBottom;
	}
...
void CDevGDI::DrawMkr(const HDC &memDC, const double *pdData, const int &iDataLen, const int &iMkrID, const int &nWaveID)
{
	...
	{
		_tcscpy(szMkrVal, m_czMkrVal[nWaveID][iMkrID]);
	}
	...
	if (m_iActiveMkr==iMkrID)	// 显示mkr横坐标值，在屏幕左上角
	{
		...
		//	if (abs(static_cast<int>(dMkrXVal))<fabs(dMkrXVal))
			{
				dMkrXVal=dMkrXVal/MHZ_1;
				_stprintf(czMkrXVal, _T("%.6fMHz"), dMkrXVal);
			}
			//else
			//{
...
// 绘制Mkr
void CDevGDI::DrawReportMkr(const HDC &memDC, const double *pdData, const int &iDataLen, const int &iMkrID, const int &nWaveID)
{
	assert((iMkrID>-1)&&(iMkrID<I_MKR_NUMBER));
	assert((nWaveID>-1)&&(nWaveID<I_WAVE_NUM));
	...
	if ((m_rect.right - m_rect.left) > 2048) {
		nLenX = (m_rect.right - m_rect.left)/500;
		nLenY = (m_rect.bottom - m_rect.top)/400;
	}

	HPEN hPen = CreatePen(PS_SOLID, nLenX, m_mkrColor[iMkrID][nWaveID]);
	HPEN hOldPen = (HPEN)SelectObject(memDC, hPen);	// 备份当前画笔设备
	double dLength = m_rect.right-m_rect.left-m_iBorderGridLeft-m_iBorderGridRight;
	double dMkrVal = 0.0;
	double dMkrXVal = 0.0;
	...
	else
	{
		...
		case DEV_MKR_TRIANGLE:	// 三角形
			MoveToEx(memDC, iXVal, iYValue, NULL);
			LineTo(memDC, iXVal+5*nLenX, iYValue-10*nLenY);
			LineTo(memDC, iXVal-5*nLenX, iYValue-10*nLenY);
			LineTo(memDC, iXVal, iYValue);
			break;
		case DEV_MKR_DIAMOND:	// 菱形
			MoveToEx(memDC, iXVal, iYValue, NULL);
			LineTo(memDC, iXVal+4*nLenX, iYValue-7*nLenY);
			LineTo(memDC, iXVal, iYValue-14);
			LineTo(memDC, iXVal-4*nLenX, iYValue-7*nLenY);
			LineTo(memDC, iXVal, iYValue);
			break;
	...
	// 显示mkr的id号
	_itot(iMkrID+1, szMkrVal, 10);
	TextOut(memDC, iXVal+6*nLenX, iYValue-10*nLenY-20, szMkrVal, _tcslen(szMkrVal));
	if (iYValue<30)		// 显示mkr的Y值
	{
		iYValue += 35;
	}
	...
	if (m_bEnableMrkYText)		// 是否显示Mkr纵坐标文字
	{
		...
		{
			TextOut(memDC, iXVal-static_cast<int>(size.cx*0.5), iYValue-10*nLenY-size.cy-10, szMkrVal, _tcslen(szMkrVal));
		}
	}
...
//设置报表绘图区域
bool CDevGDI::GDI_SetRectReport(const RECT &ReportRect, const POINT &StartPoint,const POINT &EndPoint)
{
	m_rect.left = ReportRect.left+StartPoint.x;
	m_rect.top = ReportRect.top+StartPoint.y;

	m_rect.right = ReportRect.right-EndPoint.x;	
	m_rect.bottom = ReportRect.bottom-EndPoint.y;
...
bool CDevGDI::GDI_SetMkrVal(const TCHAR* pszValY, const int &nID)
{
	...
	_tcscpy(m_czMkrVal[0][nID], pszValY);

	return true;
}

// 设置指定mkr纵坐标值, 包括波形
bool CDevGDI::GDI_SetMkrValEx(const TCHAR* pszValY, const int &nID, const int& nWaveID)
{
	assert((nID>-1)&&(nID<I_MKR_NUMBER));
	assert(pszValY!=NULL);
	if ((nID<0)||(nID>=I_MKR_NUMBER))
	...
	_tcscpy(m_czMkrVal[nWaveID][nID], pszValY);
...
// bEnableView -- 是否显示当前自定义文字
// 返回值 -- true成功，false -- 失败
bool CDevGDI::GDI_SetManualTextEx(const int &nID, const TCHAR *pczTitle, const TCHAR *pczDetail, const TCHAR *pczSeperate, 
	const bool &bEnableView, const UINT &nTextPos, int nSize, COLORREF cr)
{
	assert((nID>=0)&&(nID<=N_MAX_MANUAL_TEXT_NUMBER));
	if ((nID<0)||(nID>N_MAX_MANUAL_TEXT_NUMBER))
	{
	...
	m_ManualText[nID].nSize = nSize;
	m_ManualText[nID].cr = cr;
	return true;
}

bool CDevGDI::GDI_SetManualTextSize(int nID, int nSize)
{
	assert((nID>=0)&&(nID<=N_MAX_MANUAL_TEXT_NUMBER));
	if ((nID<0)||(nID>N_MAX_MANUAL_TEXT_NUMBER))
	{
...
bool CDevGDI::GDI_SetManualTextColor( const int &nID, COLORREF cr )
{
	assert((nID>=0)&&(nID<=N_MAX_MANUAL_TEXT_NUMBER));
	if ((nID<0)||(nID>N_MAX_MANUAL_TEXT_NUMBER))
	{
...
bool CDevGDI::GDI_GetManualTextSize( int nID, int& nSize )
{
	assert((nID>=0)&&(nID<=N_MAX_MANUAL_TEXT_NUMBER));
	if ((nID<0)||(nID>N_MAX_MANUAL_TEXT_NUMBER))
	{
...
void CDevGDI::ShowManualText(const HDC &memDC)
{
	...
	for (; i<N_MAX_MANUAL_TEXT_NUMBER; i++)
	{
		if (m_ManualText[i].bView)
		{
			...
			myFont.CreateFont(
				-MulDiv(m_ManualText[i].nSize, GetDeviceCaps(m_hdc, LOGPIXELSY), 72),                        // nHeight
				0,                         // nWidth
				0,                         // nEscapement
				0,                         // nOrientation
				FW_NORMAL,                 // nWeight
				...
				OUT_STRING_PRECIS/*OUT_DEFAULT_PRECIS*/,        // nOutPrecision
				CLIP_DEFAULT_PRECIS,       // nClipPrecision
				DEFAULT_QUALITY,           // nQuality
				DEFAULT_PITCH | FF_SWISS,  // nPitchAndFamily
				_T("Arial"));                 // lpszFacename  Arial

			hOldFont = (HFONT)SelectObject(memDC, myFont);

			// 计算字符的像素长度
			GetTextExtentPoint32( memDC, szStr, szStr.GetLength(), &SizeStr );
			SetTextColor(memDC, m_ManualText[i].cr);

			switch ( m_ManualText[i].nTextPos )
			{
			case DEVGDI_TEXT_LEFTTOP:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft, m_rect.top+m_iBorderGridTop, szStr, szStr.GetLength());
				break;
			...
			case DEVGDI_TEXT_LEFTBOTTOM:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft, m_rect.bottom-m_iBorderGridBottom-20, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_RIGHTTOP:
				TextOut(memDC, m_rect.right-m_iBorderGridRight-SizeStr.cx, m_rect.top+m_iBorderGridTop, szStr, szStr.GetLength());
				break;
			...
			case DEVGDI_TEXT_RIGHTBOTTOM:
				TextOut(memDC, m_rect.right-m_iBorderGridRight-SizeStr.cx, m_rect.bottom-m_iBorderGridBottom-20, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_CENTER:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft+(m_rect.right-m_rect.left)/2-SizeStr.cx/2, m_rect.top+m_iBorderGridTop+(m_rect.bottom - m_rect.top)/2, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_CENTERTOP:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft+(m_rect.right-m_rect.left)/2-SizeStr.cx/2, m_rect.top+m_iBorderGridTop, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_CENTERBOTTOM:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft+(m_rect.right-m_rect.left)/2-SizeStr.cx/2, m_rect.bottom-m_iBorderGridBottom-SizeStr.cy, szStr, szStr.GetLength());
				break;

			case DEVGDI_TEXT_OUTLEFTTOP:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft, m_rect.top+m_iBorderGridTop-SizeStr.cy, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTRIGHTTOP:
				TextOut(memDC, m_rect.right-m_iBorderGridRight-SizeStr.cx, m_rect.top+m_iBorderGridTop-SizeStr.cy, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTLEFTBOTTOM:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft, m_rect.bottom-m_iBorderGridBottom, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTRIGHTBOTTOM:
				TextOut(memDC, m_rect.right-m_iBorderGridRight-SizeStr.cx, m_rect.bottom-m_iBorderGridBottom, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTCENTTOP:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft+(m_rect.right-m_rect.left)/2-SizeStr.cx/2, 
					m_rect.top+m_iBorderGridTop-SizeStr.cy, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTCENTBOTTOM:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft+(m_rect.right-m_rect.left)/2-SizeStr.cx/2, 
					m_rect.bottom-m_iBorderGridBottom, szStr, szStr.GetLength());
				break;

			case DEVGDI_TEXT_OUTERLEFTTOP:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft, m_rect.top+m_iBorderGridTop-SizeStr.cy*2, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTERRIGHTTOP:
				TextOut(memDC, m_rect.right-m_iBorderGridRight-SizeStr.cx, m_rect.top+m_iBorderGridTop-SizeStr.cy*2, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTERCENTTOP:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft+(m_rect.right-m_rect.left)/2-SizeStr.cx/2, 
					m_rect.top+m_iBorderGridTop-SizeStr.cy*2, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTERLEFTBOTTOM:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft, m_rect.bottom-m_iBorderGridBottom+SizeStr.cy, szStr, szStr.GetLength());
				break;
			case DEVGDI_TEXT_OUTERRIGHTBOTTOM:
				...
				break;
			case DEVGDI_TEXT_OUTERCENTBOTTOM:
				TextOut(memDC, m_rect.left+m_iBorderGridLeft+(m_rect.right-m_rect.left)/2-SizeStr.cx/2, 
					m_rect.bottom-m_iBorderGridBottom+SizeStr.cy, szStr, szStr.GetLength());
				break;
...
// 显示自定义文字
// 修改 wangzheng 2008-06-19
void CDevGDI::ShowManualTextForPrint(const HDC &memDC)
{
	...
	const int &nColorNum)
{
	...
	// 绘制光谱图
	//nStartY = m_rect.bottom-m_iBorderGridBottom;
	nStartY = m_rect.top+m_iBorderGridTop;
	for (i=0; i<nWaveNum; i++)
	{
		nStartX = m_rect.left+m_iBorderGridLeft;
		//nEndY = m_rect.bottom-m_iBorderGridBottom-static_cast<int>(dWaveHeight*(i+1));
		nEndY = m_rect.top+m_iBorderGridTop+static_cast<int>(dWaveHeight*(i+1));
...
void CDevGDI::GDI_EnableSpectrogramMkrLine(bool IsEnable) {
	m_IsSpectrogramMkrLine = IsEnable;
}
...
void CDevGDI::DrawSpectrogramMkr(HDC &memDC, const int &nDataLen, const int &nWaveNum)
{
	...
	if (m_bSpectrogramMkrX)	// 绘制横向mkr
	{
		...
		if (m_IsSpectrogramMkrLine) {
			MoveToEx(memDC, nXPos,m_rect.top+m_iBorderGridTop, NULL);
			LineTo(memDC, nXPos, m_rect.bottom-m_iBorderGridBottom);
		}
	}
	...
	if (m_bSpectrogramMkrY)	// 绘制纵向mkr
	{
		...
		if (m_IsSpectrogramMkrLine) {
			MoveToEx(memDC, m_rect.left+m_iBorderGridLeft,nYPos, NULL);
			LineTo(memDC, m_rect.right-m_iBorderGridRight, nYPos);
		}
...
void CDevGDI::DrawSpecgView(HDC &memDC, const COLORREF *pColor, const double *pdLevel, const int &nColorNum)
{
	...
	double Heigth = (double) (m_rect.bottom-m_iBorderGridBottom- m_rect.top-m_iBorderGridTop)/nColorNum;
	//double Width = (double)(m_rect.right - )
	for (i=0; i<nColorNum; i++)
	{
		//nXPos = m_rect.right-m_rect.left-m_iBorderGridRight;//+24;
		nXPos = m_rect.left+20;//-m_iBorderGridLeft;//+24;
		nYPos = m_rect.bottom-m_iBorderGridBottom-Heigth*i;
		nXStopPos = m_rect.left +m_iBorderGridLeft-2;
		Rectangle(memDC, nXPos, nYPos-Heigth, nXStopPos, nYPos);	// 绘制光谱颜色表
	...
	_tcscpy(czTemp, _T("80dBµV"));
	TextOut(memDC, nXPos-10, m_rect.top+5, czTemp, _tcslen(czTemp));

	_tcscpy(czTemp, _T("-20"));
	TextOut(memDC, nXPos-10, m_rect.bottom-18, czTemp, _tcslen(czTemp));
...
void CDevGDI::GDI_SetRefLine( int nLineID, bool IsEnable, COLORREF cr, 
	const INT64& iFStart, const INT64& iFStop,
	const double& dY, TCHAR* szText, int nTextPos )
{
	double v[4] = {0.0};
	int nLineWidth = 2;
	...
	if ( IsEnable )
	{
		CRect rc = CRect(m_rect);
		GDI_SetLimitLineFullScreen(false, nLineID);
		GDI_EnableLimitLine(true, nLineID);		
		v[0] = static_cast<double>(iFStart);
		v[1] = static_cast<double>(iFStop);
		v[2] = dY;
		v[3] = dY;
		GDI_SetLimitLineVal(v[0], v[1], v[2], v[3], nLineID);
		GDI_SetLimitTextDock(nTextPos, nLineID);
		GDI_SetManualLimitText(szText,nLineID);

		if (rc.Width() > 1050) {
			nLineWidth = (int)(2*(CRect(m_rect).Width()/860.0));
		}

		GDI_SetLimitLineColor(cr, nLineID, nLineWidth);
		GDI_EnableLimitLineText(TRUE, nLineID);
		GDI_EnableLimitManualText(TRUE, nLineID);
	}
	else
	{
		GDI_SetLimitLineFullScreen(true, nLineID);
		GDI_EnableLimitLine(false, nLineID);		
	}
}

void CDevGDI::GDI_SetRectangles( tagRectangle* pRectangles, size_t nRectangles )
{
	if (pRectangles != NULL)
	{
		if (nRectangles <= N_RECT_NUM) {
			m_nRectangles = nRectangles;

			memcpy(m_Rectangles, pRectangles, nRectangles*sizeof(tagRectangle));
		}
	}
}

void CDevGDI::DrawRectangles(const HDC& memDC, int nDataLen)
{
	
	size_t uSize = m_nRectangles;
	for (size_t i = 0; i<uSize; i++)
	{
		if (m_Rectangles[i].IsEnable) {
			double dcrPercent = m_Rectangles[i].dPercent;
			COLORREF crFill = RGB(GetRValue(m_bkColor)*(1-dcrPercent) + GetRValue(m_Rectangles[i].crColor)*dcrPercent, 
				GetGValue(m_bkColor)*(1-dcrPercent) + GetGValue(m_Rectangles[i].crColor)*dcrPercent,
				GetBValue(m_bkColor)*(1-dcrPercent) + GetBValue(m_Rectangles[i].crColor)*dcrPercent);

			LOGBRUSH myLogBrush;
			HPEN hPen = CreatePen(PS_SOLID, m_iGridBorderWidth, crFill);
			HPEN hOldPen = (HPEN)SelectObject(memDC, hPen);	// 备份当前画笔设备
			myLogBrush.lbHatch = HS_CROSS;
			myLogBrush.lbStyle = BS_SOLID;
			myLogBrush.lbColor = crFill;
			HBRUSH myBrush = CreateBrushIndirect(&myLogBrush);
			hOldPen = (HPEN)SelectObject(memDC, myBrush);// 备份当前画笔设备
			...
			int nXPosTop = GetWaveLineX(m_Rectangles[i].nXTop);
			int nYPosTop = GetWaveLineY(m_Rectangles[i].dYTop);
			int nXPosBottom = GetWaveLineX(m_Rectangles[i].nXBottom);
			int nYPosBottom = GetWaveLineY(m_Rectangles[i].dYBottom);

			Rectangle(memDC, nXPosTop, nYPosTop, nXPosBottom, nYPosBottom);

			// 写文字
			if ( _tcslen(m_Rectangles[i].tc) > 0  )
				TextOut(memDC, nXPosTop, nYPosTop+m_Rectangles[i].nYOffset, m_Rectangles[i].tc, _tcslen(m_Rectangles[i].tc));

			if ( _tcslen(m_Rectangles[i].tc1) > 0 )
			{
				SIZE SizeStr;
				GetTextExtentPoint32( memDC, m_Rectangles[i].tc1,  _tcslen(m_Rectangles[i].tc1), &SizeStr );
				TextOut(memDC, nXPosBottom-SizeStr.cx, nYPosTop+m_Rectangles[i].nYOffset, m_Rectangles[i].tc1, _tcslen(m_Rectangles[i].tc1));
			}
...
// 设置手动表格
void CDevGDI::GDI_SetManualTable( tagManualTable* pManualTable, int nTableID )
{
	if (nTableID < N_TABLE_NUM) {
		memcpy(&m_ManualTable[nTableID], pManualTable, sizeof(tagManualTable));
	}
}

// 设置手动表格的文字和颜色
void CDevGDI::GDI_SetManualTableItem(int nItem, int nSubItem, int nTableID, tagCell* tCell) {
	if ((nItem < N_MAX_MANUAL_TABLE_ROW) && (nSubItem < N_MAX_MANUAL_TABLE_COL) && (nTableID < N_TABLE_NUM)) {
		memcpy(&(m_ManualTable[nTableID].tc[nItem][nSubItem]), tCell, sizeof(tagCell));
	}
}

// 设置手动表格文字
void CDevGDI::GDI_SetManualTableText(int nItem, int nSubItem, int nTableID, TCHAR* szCell) {
	if ((nItem < N_MAX_MANUAL_TABLE_ROW) && (nSubItem < N_MAX_MANUAL_TABLE_COL) && (nTableID < N_TABLE_NUM)) {
		_tcscpy_s((m_ManualTable[nTableID].tc[nItem][nSubItem].tc),100, szCell);
	}
}

// 绘制自定义表格
void CDevGDI::DrawManualTable( const HDC& memDC )
{
	for (int i=0; i<N_TABLE_NUM; i++) {
		if (m_ManualTable[i].IsEnable) {
			int nStartX, nStartY, nStopX, nStopY, nSpanX, nSpanY;
			int nSpitX, nSpitY;

			switch(i) {
			case DEVGDI_TAB_LEFT:
				{
					nStartX = m_rect.left + m_ManualTable[i].rcBorder.left;
					nStartY = m_rect.top+m_ManualTable[i].rcBorder.top;
					nStopX = m_rect.left + m_iBorderGridLeft-m_ManualTable[i].rcBorder.right;
					nStopY = m_rect.bottom-m_ManualTable[i].rcBorder.bottom;

					nSpanX = nStopX - nStartX;
					nSpanY = nStopY - nStartY;

					nSpitX = nSpanX / m_ManualTable[i].nCols;
					nSpitY = nSpanY / m_ManualTable[i].nRows;
				}				
				break;
			case DEVGDI_TAB_TOP:
				{
					nStartX = m_rect.left + m_ManualTable[i].rcBorder.left;
					nStartY = m_rect.top+m_ManualTable[i].rcBorder.top;
					nStopX = m_rect.right - m_ManualTable[i].rcBorder.right;
					nStopY = m_rect.top+m_iBorderGridTop-m_ManualTable[i].rcBorder.bottom;
				...
				break;
			case DEVGDI_TAB_RIGHT:
				{
					nStartX = m_rect.right - m_iBorderGridRight + m_ManualTable[i].rcBorder.left;
					nStartY = m_rect.top+m_ManualTable[i].rcBorder.top;
					nStopX = m_rect.right - m_ManualTable[i].rcBorder.right;
					nStopY = m_rect.bottom-m_ManualTable[i].rcBorder.bottom;
				...
				break;
			case DEVGDI_TAB_BOTTOM:
				{
					nStartX = m_rect.left + m_ManualTable[i].rcBorder.left;
					nStartY = m_rect.bottom - m_iBorderGridBottom + m_ManualTable[i].rcBorder.top;
					nStopX = m_rect.right - m_ManualTable[i].rcBorder.right;
					nStopY = m_rect.bottom-m_ManualTable[i].rcBorder.bottom;
			...
			// 当前画笔设备 -- Grid
			LOGBRUSH myLogBrush;
			HPEN hPen = CreatePen(PS_SOLID, m_iGridBorderWidth, m_ManualTable[i].cr);
			HPEN hOldPen = (HPEN)SelectObject(memDC, hPen);	// 备份当前画笔设备
			myLogBrush.lbHatch = HS_CROSS;
			myLogBrush.lbStyle = BS_HOLLOW;
			myLogBrush.lbColor = m_GridColor;
			...
			// 绘制删格外边框
			Rectangle(memDC, nStartX, nStartY,nStopX, nStopY);
			...
			hOldPen = (HPEN)SelectObject(memDC, hPen);	// 备份当前画笔设备
			//Rectangle(memDC, nStartX, nStartY,nStopX, nStopY);
			for (int nRow=0; nRow<m_ManualTable[i].nRows; nRow++) {
				MoveToEx(memDC, nStartX, nStartY+nRow*nSpitY, NULL);
				LineTo(memDC, nStopX, nStartY+nRow*nSpitY);
			}

			for (int nCol=0; nCol<m_ManualTable[i].nCols; nCol++) {
				MoveToEx(memDC, nStartX+nCol*nSpitX, nStartY, NULL);
				LineTo(memDC, nStartX+nCol*nSpitX, nStopY);
			}
			...
			for (int nRow=0; nRow<m_ManualTable[i].nRows; nRow++) {
				for (int nCol=0; nCol<m_ManualTable[i].nCols; nCol++) {
					hPen = CreatePen(PS_SOLID, m_iGridLineWidth, m_ManualTable[i].tc[nRow][nCol].cr);
					hOldPen = (HPEN)SelectObject(memDC, hPen);	// 备份当前画笔设备

					TextOut(memDC, nStartX+nCol*nSpitX+2, nStartY+nRow*nSpitY+1, m_ManualTable[i].tc[nRow][nCol].tc, _tcslen(m_ManualTable[i].tc[nRow][nCol].tc));
...
// 显示两个波形窗口histogram
bool CDevGDI::GDI_DrawHistogram(const double *pdfre,const double *pdLevel,const int &nDotNum,const COLORREF &HistogramColor)
{
	int i = 0;
	...
	DrawHistogram(memDC,pdfre,pdLevel,nDotNum,HistogramColor);


	DrawHistogramGrid(memDC);
...
bool CDevGDI::DrawHistogram(HDC &memDC,const double *pdfre,const double *pdLevel,const int &nDotNum,const COLORREF &HistogramColor)
{
	HPEN hOldPen;	// 备份当前画笔设备
	HBRUSH hOldBrush;
	LOGBRUSH pLogBrush[N_SPECTROGRAM_COLOR_NUM];
	...
	int nEndX = 0;
	// 每个光谱波形高度
	double dWaveHeight = (static_cast<double>(m_rect.bottom-m_rect.top-m_iBorderGridBottom-m_iBorderGridTop));
	// 每个光谱点的宽度
	double dWaveWidth = (static_cast<double>(m_rect.right-m_rect.left-m_iBorderGridRight-m_iBorderGridLeft-1))/nDotNum;
	...
	pLogBrush[0].lbColor = HistogramColor;
	pLogBrush[0].lbHatch = HS_CROSS;
	pLogBrush[0].lbStyle = BS_SOLID;
	pBrush[0] = CreateBrushIndirect(&pLogBrush[0]);
	phPen[0] = CreatePen(PS_SOLID, 1,HistogramColor);
	...
	// 绘制光谱图
	nStartY = m_rect.bottom-m_iBorderGridBottom;
	nStartX = m_rect.left+m_iBorderGridLeft+5;

	SelectObject(memDC, phPen[0]);	// 根据幅度值选择颜色
	SelectObject(memDC, pBrush[0]);

	for (i=0; i<nDotNum; i++)
	{


	//	nEndY = m_rect.bottom-m_iBorderGridBottom-dWaveHeight;//60;//static_cast<int>(pdLevel[i]);
		nEndY=GetWaveLineY(pdLevel[i]);
		nEndX = nStartX+dWaveWidth-4;
		Rectangle(memDC, nStartX, nStartY, nEndX, nEndY);
		ShowText(memDC, pdLevel[i], L"", nStartX+6, nEndY-15, 0);
		ShowText(memDC, pdfre[i], L"MHz", nStartX+6, nEndY-30, 0);
		nStartX = nStartX+dWaveWidth;
	}

	DeleteObject(phPen[0]);	// 卸载颜色表
	DeleteObject(pBrush[0]);
	SelectObject(memDC, hOldBrush);	// 回复画笔
	SelectObject(memDC, hOldPen);
...
// 绘制表格函数
bool CDevGDI::DrawHistogramGrid(const HDC &memDC)
{
	int i = 0;
	double dPosX = 0.0;
	double dPosY = 0.0;
