...
enum DEVGDI_MANUALTEXT_POS	// 手工文字位置	wangzheng 2008-06-19
{
	...
	DEVGDI_TEXT_RIGHTBOTTOM = 5,
	DEVGDI_TEXT_CENTER = 6,
	DEVGDI_TEXT_CENTERTOP = 7,
	DEVGDI_TEXT_CENTERBOTTOM = 8,

	DEVGDI_TEXT_OUTLEFTTOP,
	DEVGDI_TEXT_OUTRIGHTTOP,
	DEVGDI_TEXT_OUTCENTTOP,
	DEVGDI_TEXT_OUTLEFTBOTTOM,
	DEVGDI_TEXT_OUTRIGHTBOTTOM,
	DEVGDI_TEXT_OUTCENTBOTTOM,

	DEVGDI_TEXT_OUTERLEFTTOP,
	DEVGDI_TEXT_OUTERRIGHTTOP,
	DEVGDI_TEXT_OUTERCENTTOP,
	DEVGDI_TEXT_OUTERLEFTBOTTOM,
	DEVGDI_TEXT_OUTERRIGHTBOTTOM,
	DEVGDI_TEXT_OUTERCENTBOTTOM
};

// 手动表格的位置
enum DEVGDI_MANUAL_TABLE_POS	{
	DEVGDI_TAB_LEFT = 0,
	DEVGDI_TAB_TOP = 1,
	DEVGDI_TAB_RIGHT = 2,
	DEVGDI_TAB_BOTTOM = 3
...
const static int I_DEPARTMENT_LEN = 1000;
const static int I_LIMITLINE_NUMBER = 3000;
const static int I_MKR_NUMBER = 10;
const static int I_WAVE_NUM = 20;
const static int N_MAX_MANUAL_TEXT_NUMBER = 50;
const static int N_SPECTROGRAM_COLOR_NUM = 256;	// 光谱图最大颜色数量支持
const static int N_RECT_NUM = 20;				// 最大半透明矩形框数量
const static int N_TABLE_NUM = 4;
const static int N_MAX_MANUAL_TABLE_ROW = 30;	// 最大自定义表格行数
const static int N_MAX_MANUAL_TABLE_COL = 10;	// 最大自定义表格列数
...
struct tagRectangle
{
	...
	double dYTop;	// 左上角的点Y
	int nXBottom;	// 右下角的点X
	double dYBottom; // 右下角的点Y
	COLORREF crColor; // 该矩形颜色
	TCHAR tc[20];
	int nYOffset;
	TCHAR tc1[20];
	int nYOffset1;
...
// 手动表格, 只能有4个，分别位于上下左右
struct tagManualTable {
	BOOL IsEnable;	// 表格使能
	int nRows;	// 行数
	int nCols;  // 列数
	COLORREF cr;// 表格颜色
	CRect rcBorder; // 四边距，相对于主绘图区和绘图边框
	tagCell tc[N_MAX_MANUAL_TABLE_ROW][N_MAX_MANUAL_TABLE_COL]; // 表格每行列的内容
};

class CDevGDI
{
public:
	bool GDI_DrawHistogram(const double *pdfre,const double *pdLevel,const int &nDotNum,const COLORREF &HistogramColor);
	...
	void GDI_SetGridColor(const COLORREF &GridColor);
	// Text ///////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	// 设置光谱图文字说明开关
	...
	bool GDI_SetManualTextEx(const int &nID, const TCHAR *pczTitle, const TCHAR *pczDetail, const TCHAR *pczSeperate,
		const bool &bEnableView, const UINT &nTextPos, int nSize, COLORREF cr);
	bool GDI_SetManualTextColor(const int &nID,  COLORREF cr);
	bool GDI_SetManualTextSize(int nID, int nSize);			// 设置自定义文字大小
	bool GDI_GetManualTextSize(int nID, int& nSize);			// 设置自定义文字大小
	...
	// 读取光谱图mkr line
	void GDI_EnableSpectrogramMkrLine(bool IsEnable);
	// 设置光谱图Mkr开关状态
	// bXEnable -- 横向mkr开关
	// bYEnable -- 纵向Mkr开关
	...
	bool GDI_SetMkrValEx(const TCHAR* pszValY, const int &nID, const int& nWaveID); // 设置指定mkr纵坐标值
	...
	void GDI_SetMkrColor(const COLORREF &mkrColor, const int &nMkrID );
	// 获得当前频标的值
	// dYValue -- 频标的横坐标值
	// dXValue -- 频标的纵坐标值
	// nMkrID -- 获得当前频标的id
	...
	// 设置曲线颜色
	void GDI_SetLineColor(int nColor, COLORREF cr);
	// 设置曲线颜色
	COLORREF GDI_GetLineColor(int nColor);
	// 显示QAM数据
	// pnData -- 波形数据
	// nDataLen -- 波形数据长度
	...
	bool GDI_SetRectReport(const RECT &ReportRect, const POINT &StartPoint,const POINT &EndPoint);
	// 绘制参考线
	void GDI_SetRefLine(int nLineID, bool IsEnable, COLORREF cr, const INT64& iFStart, const INT64& iFStop, const double& dY, TCHAR* szText, int nTextPos = DEVGDI_TEXT_RIGHT);
	...
	// 设置半透明矩形
	void GDI_SetRectangles( tagRectangle* pRectangles, size_t nRectangles );

	// 设置手动表格
	void GDI_SetManualTable(tagManualTable* pManualTable, int nTableID);
	void GDI_SetManualTableItem(int nItem, int nSubItem, int nTableID, tagCell* tCell);
	void GDI_SetManualTableText(int nItem, int nSubItem, int nTableID, TCHAR* szCell);
...
protected:
	...
	bool DrawHistogram(HDC &memDC,const double *pdfre,const double *pdLevel,const int &nDotNum,const COLORREF &HistogramColor);
	bool DrawHistogramGrid(const HDC &memDC);
	// MKR ///////////////////////////////////////////////////////////////////////////
	//
	// 显示全部Mkr的函数
...
private:
	...
	void DrawReportMkr(const HDC &memDC, const double *pdData, const int &iDataLen, const int &iMkrID, const int &nWaveID);
	void ShowManualTextForPrint(const HDC &memDC);

	// 绘制半透明矩形框
	void DrawRectangles(const HDC& memDC, int nDataLen);

	// 绘制自定义表格
	void DrawManualTable(const HDC& memDC);
	...
	TCHAR m_czMkrVal[I_WAVE_NUM][I_MKR_NUMBER][N_TEXT_LEN];	// 手动mkr的值
	...
	tagRectangle m_Rectangles[N_RECT_NUM];	// 半透明矩形框对象
	UINT m_nRectangles;				// 画了多少个半透明矩形

	tagManualTable m_ManualTable[N_TABLE_NUM];	// 手动表格数目
};
