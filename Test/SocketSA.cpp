...
// socket收发数据
BOOL CSocketSA::RSCmd ( const char* szCmd, DWORD dwTimeout, BYTE send[], BYTE recv[], DWORD& recs )
{
	int nCount = 0; 
	...
	do 
	{	
		this->WriteComm( (BYTE*)szCmd, strlen(szCmd), dwTimeout );
		Sleep(1);
...
BOOL CSocketSA::LanSendCmd(const char* szCmd, DWORD dwTimeout, double &dVal )
{
...
BOOL CSocketSA::LanSendCmd(const char* szCmd, DWORD dwTimeout, long &lVal )
{
	BYTE send[SEND_BUF] = {0};
	BYTE recv[RECV_BUF] = {0};
	DWORD recs;
...
BOOL CSocketSA::LanSendCmd(const char* szCmd, DWORD dwTimeout, INT64 &dVal )
{
	BYTE send[SEND_BUF] = {0};
	BYTE recv[RECV_BUF] = {0};
	DWORD recs;
...
BOOL CSocketSA::LanSendCmd(const char* szCmd, DWORD dwTimeout, int &nVal )
{
	BYTE send[SEND_BUF] = {0};
	BYTE recv[RECV_BUF] = {0};
	DWORD recs;
...
BOOL CSocketSA::LanSendCmd(const char* szCmd, DWORD dwTimeout, CString &szVal )
{
	BYTE send[SEND_BUF] = {0};
	BYTE recv[RECV_BUF] = {0};
	DWORD recs;
...
BOOL CSocketSA::LanSendCmd(const char* szCmd, DWORD dwTimeout, int &iVal, CString szRet[], int iRet  )
{
	BYTE send[SEND_BUF];
	BYTE recv[RECV_BUF];
	DWORD recs;
...
BOOL CSocketSA::LanSendCmd(const char* szCmd, DWORD dwTimeout, bool &bVal, CString szRet[], int iRet  )
{
	BYTE send[SEND_BUF];
	BYTE recv[RECV_BUF];
	DWORD recs;
...
// 通过socket设置，不需要收数的
BOOL CSocketSA::LanSendCmd(const char* szCmd, DWORD dwTimeout, bool IsEcho )
{
	DWORD dwSend = 0;
	DWORD dwRecv = 0;
...
// 检测是否有回传
BOOL CSocketSA::CheckEchoAndType(CString& strType)
{
	CString strRecv = _T("");
	BYTE bySend[100] = "*idn?";
	BYTE byRecv[200] = {0};
	...
	this->WriteComm( bySend, strlen((char*)bySend), 5000 );
	Sleep(1);
	dwRecv = this->ReadComm( byRecv, 200, 5000 );

	strRecv = CString(byRecv);
	if (strRecv.Find(_T("E8000")) >= 0) {
		strType = _T("E8000");
	}
	else {
		strType = _T("SA");
	...
	char* pdest = strstr( (char*)byRecv, (char*)bySend );
	if (pdest != NULL)
	{
		return TRUE;
	}
